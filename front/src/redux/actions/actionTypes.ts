...
export const selectActions = {
  selectCreateAccountButton: actionCreator<void>(
    "SELECT_CREATE_ACCOUNT_BUTTON"
  ),
...
): ThunkAction<Promise<void>, AppState, undefined, Action<AppState>> => {
  return async (dispatch: Dispatch<Action<any>>) => {
    dispatch(userActions.loadAllUserInfo.started({ params: {} }));
    const REGISTER_ENDPOINT =
      "https://dogress-api.herokuapp.com/api/v1/register";
    await axios
      ...
      .then(results => {
        dispatch(
          userActions.loadAllUserInfo.done({ params: {}, result: results })
        );
      })
      .catch(error => {
        dispatch(
          userActions.loadAllUserInfo.failed({ params: {}, error: error })
        );
      });
  };
};
...
export const getTodo = (): ThunkAction<
  Promise<void>,
  AppState,
  undefined,
  Action<AppState>
> => {
  return async (dispatch: Dispatch<Action<any>>) => {
    ...
    const TODO_ENDPOINT =
      "https://dogress-api.herokuapp.com/api/v1/parent_tasks";
    await axios
      .get(TODO_ENDPOINT, {
        headers: {
          Authorization: `Bearer ${
            store.getState().user.results.data.token.access_token
          }`
        }
      })
      .then(results => {
        dispatch(
          todoActions.getTodo.done({ params: {}, result: results.data.data })
        );
      })
      .catch(error => {
...
export const postTodo = (
  ...
  return async (dispatch: Dispatch<Action<any>>) => {
    ...
    await axios
      .post(TODO_ENDPOINT, data, { headers: headers })
      .then(results => {
        dispatch(
          todoActions.postTodo.done({ params: {}, result: results.data.data })
        );
      })
      .catch(error => {
...
export const putTodo = (
  ...
  return async (dispatch: Dispatch<Action<any>>) => {
    ...
    await axios
      .put(TODO_ENDPOINT, data, { headers: headers })
      .then(results => {
        dispatch(
          todoActions.putTodo.done({ params: {}, result: results.data.data })
        );
      })
      .catch(error => {
...
export const deleteTodo = (
  ...
  return async (dispatch: Dispatch<Action<any>>) => {
    ...
    await axios
      ...
      .then(results => {
        dispatch(
          todoActions.deleteTodo.done({ params: {}, result: results.data.data })
        );
      })
      .catch(error => {
